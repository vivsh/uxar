...
fn derive_bitrole_impl(input: &DeriveInput, _args: BitRoleArgs) -> Result<TokenStream, syn::Error> {
    ...
    for (idx, variant) in variants.iter().enumerate() {
        ...
        if bit_position >= 64 {
            return Err(syn::Error::new_spanned(
                variant,
                format!(
                    "Bit position {} must be < 64 for RoleType (u64)",
                    bit_position
                ),
            ));
        }

        let bit_u8 = u8::try_from(bit_position)
            .map_err(|_| syn::Error::new_spanned(variant, "Bit position must fit within u8"))?;
        ...
        mask_arms.push(quote! {
            #enum_name::#variant_ident => #mask_val as ::vyuh::auth::RoleType
        });
    }
    ...
    let expanded = quote! {
        ...
        impl #impl_generics #enum_name #ty_generics #where_clause {
            #[doc(hidden)]
            pub const fn __vyuh_mask(role: Self) -> ::vyuh::auth::RoleType {
                match role {
                    #(#mask_arms,)*
                }
            }
        ...
        impl #impl_generics ::vyuh::auth::BitRole for #enum_name #ty_generics #where_clause {
            ...
            syn::Expr::Lit(lit_expr) => {
                if let syn::Lit::Int(lit_int) = &lit_expr.lit {
                    return lit_int.base10_parse::<usize>().map_err(|_| {
                        syn::Error::new_spanned(lit_int, "Invalid discriminant value")
                    });
                }
            }
            _ => {
