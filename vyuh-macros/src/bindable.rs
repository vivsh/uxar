...
pub(crate) fn derive_bindable_impl(input: &DeriveInput) -> proc_macro2::TokenStream {
    ...
    quote! {
        impl #impl_generics ::vyuh::db::Bindable for #ident #ty_generics #where_clause {
            ...
            fn bind_values(
                &self,
                args: &mut ::vyuh::db::Arguments<'static>,
            ) -> Result<(), ::sqlx::Error> {
                #(#bind_stmts)*
                Ok(())
            }
...
fn gen_where_clause(generics: &mut syn::Generics, fields: &[FieldMeta]) {
    ...
    for field in fields {
        ...
        if is_flatten(field) {
            ...
            wc.predicates.push(syn::parse_quote! {
                #ty: ::core::clone::Clone
                    + for<'q> ::sqlx::Encode<'q, ::vyuh::db::Database>
                + ::sqlx::Type<::vyuh::db::Database>
                + ::core::marker::Send
            });
        }
    }
...
fn gen_flatten_bind(ident: &syn::Ident, ty: &Type) -> proc_macro2::TokenStream {
    quote! {
        <#ty as ::vyuh::db::Bindable>::bind_values(&self.#ident, args)?;
    }
}
...
fn gen_json_bind(ident: &syn::Ident) -> proc_macro2::TokenStream {
    quote! {
        {
            let value = ::vyuh::db::serde_json::to_value(&self.#ident)
...
fn gen_bind_column_names(fields: &[FieldMeta]) -> Vec<proc_macro2::TokenStream> {
    ...
    for field in fields {
        ...
        if is_flatten(field) {
            let ty = &field.ty;
            stmts.push(quote! {
                cols.extend(<#ty as ::vyuh::db::Bindable>::bind_column_names());
            });
        } else {
            let col_name = field
                .column
