...
///
/// Maps to vyuh::signals::SignalConf runtime structure.
...
pub(crate) fn parse_signal(attr: TokenStream, item: TokenStream) -> TokenStream {
    bundlepart::generate_bundle_part::<SignalConfMeta>(attr, item, "signal", build_signal_conf)
}
...
fn build_signal_conf(
    ...
    Ok(quote! {
        ::vyuh::signals::SignalConf::default()
    })
}
