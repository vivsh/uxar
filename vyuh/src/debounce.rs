use crate::callables::{CallError, Callable, PayloadData};
...
mod tests {
    ...
    fn leading() -> Debouncer {
        Debouncer::new(DebounceConf {
            mode: DebounceMode::Leading,
            interval: INTERVAL,
        })
    }
    fn trailing() -> Debouncer {
        Debouncer::new(DebounceConf {
    ...
    E: fmt::Debug + From<CallError> + Send + 'static,
{
    ...
    loop {
        ...
        match decision {
            ...
            DebounceDecision::SleepUntil(Some(t)) => {
                sleep.as_mut().reset(t);
                timer_active = true;
            }
            DebounceDecision::SleepUntil(None) => {
                timer_active = false;
            }
            DebounceDecision::Nothing => {}
        }
...
mod call_tests {
    ...
    async fn both_burst_fires_leading_and_one_trailing() {
        ...
        assert_eq!(
            ...
            2,
            "trailing fires once after quiet"
        );
    }
    ...
    async fn clone_shares_background_task() {
        ...
        assert_eq!(
            c.load(Ordering::SeqCst),
            2,
            "clone fires after shared cooldown expires"
        );
    }
    ...
    async fn trailing_runs_are_sequential() {
        ...
        assert_eq!(
            ...
            3,
            "handler should run once per quiet period"
        );
        assert!(
            !overlap_seen.load(Ordering::SeqCst),
            "runs must never overlap"
        );
    }
